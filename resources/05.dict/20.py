...
def words(text):
    ...
    text_dict = {}
    new_list = []
    text_replace = text.replace(',',' ').replace('?',' ').replace('.',' ').replace(':',' ').replace(';',' ')
    ...
    for char in new_set:
        if len(char) not in text_dict:
            text_dict[len(char)] = [char]
        else:
            text_dict[len(char)].append(char)
    for k in sorted(text_dict):
        print(f'Words of length {k}:')
        print('    ' + str(sorted(text_dict[k])))
