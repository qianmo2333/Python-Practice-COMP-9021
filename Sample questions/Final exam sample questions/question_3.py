...
def f(m, n, direction):
    ...
    m_str, n_str = str(m), str(n)
    max_length = max(len(m_str), len(n_str))
    m_str, n_str = m_str.zfill(max_length), n_str.zfill(max_length)

    results = []
    for _ in range(max_length):
        result = int(m_str) + int(n_str)
        results.append(result)
        m_str, n_str = rotate(m_str, direction), rotate(n_str, direction)

    max_result_length = len(str(max(results)))

    for result in results:
        result_str = str(result).zfill(max_result_length)
        print(f"{m_str} plus {n_str} equals {result_str}")
        m_str, n_str = rotate(m_str, direction), rotate(n_str, direction)
