...
def f(arg_for_seed, nb_of_elements, max_element):
    ...
    for i in range(len(L)):
        if not temp or L[i] != temp[-1]:
            temp.append(L[i])
    current_sequence = []
    R = []
    for i in range(len(temp)):
        if not current_sequence or temp[i] > current_sequence[-1]:
            current_sequence.append(temp[i])
        else:
            R.append(current_sequence)
            current_sequence = [temp[i]]
    if current_sequence:
        R.append(current_sequence)
