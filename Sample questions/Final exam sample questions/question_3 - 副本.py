...
def f(m, n, direction):
    ...
    m_str = str(m)
    n_str = str(n)
    max_length = max(len(m_str),len(n_str))
    m_str, n_str = m_str.zfill(max_length),n_str.zfill(max_length)
    result = []
    for _ in range(len(m_str)):
        sum = int(m_str) + int(n_str)
        result.append(sum)
        m_str = rotation(m_str,direction)
        n_str = rotation(n_str,direction)
    max_result_length = len(str(max(result)))
    for _ in range(len(m_str)):
        sum = int(m_str) + int(n_str)
        str_sum = str(sum).zfill(max_result_length)
        print(f'{m_str} plus {n_str} equals {str_sum}')
        m_str = rotation(m_str,direction)
        n_str = rotation(n_str,direction)
